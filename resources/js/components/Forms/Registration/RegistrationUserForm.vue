    ...
    <q-form ref="form" autocomplete="off" class="form" @submit.prevent="submit">
        <div class="form-inputs">
            <q-input v-model="formData.full_name"
                :rules="[v => v.length >= 2 || `The name must have at least 2 letters`]" hide-bottom-space label="Name"
                required />

            <q-input v-model="formData.email" autocomplete="off" label="Email" required type="email" />
            <q-input v-model="formData.password" autocomplete="off" label="Password" required type="password" />
            <q-input v-model="formData.password_confirmation"
                :rules="[v => v === formData.password || `The passwords are not the same`]" hide-bottom-space
                label="Repeat the password" required type="password" />
        </div>
