...
const submit = async () => {
    loading.value = true
    try {
        await outletsStore.fetchEditOutlet(props.outlet.id, outlet)
    } catch (err) {
...
<template>
    <FormWrapper :loading="loading" action-label="Edit" title="Edit outlet" @cancel="cancel" @submit="submit">
        <q-input
            v-model="outlet.name"
            :rules="[v => v.length >= 2 || `The point name must have at least 2 letters`]"
            hide-bottom-space
            label="Name"
